...
const kpis = [
  { label: 'Готовность к переходу', value: '68%', delta: '+4%', color: '#005DB9', bg: '#E8F0FA' },
  { label: 'Пройдено курсов', value: '7', delta: '+2 в квартале', color: '#10B981', bg: '#D1FAE5' },
  { label: 'Часов обучения', value: '124', delta: '+18 ч', color: '#8B5CF6', bg: '#EDE9FE' },
  { label: 'Вовлеченность', value: '84%', delta: 'Высокая', color: '#F59E0B', bg: '#FEF3C7' },
];
        ...
        {kpis.map((kpi, i) => (
          <div key={i} className="bg-white rounded-2xl border border-border p-4 shadow-card">
            <div className="flex items-start justify-between mb-2">
              <p className="text-xs text-secondary leading-tight">{kpi.label}</p>
              <div className="w-7 h-7 rounded-lg flex-shrink-0" style={{ backgroundColor: kpi.bg }} />
            </div>
            <p className="text-xl font-bold text-dark">{kpi.value}</p>
            <p className="text-xs mt-0.5 font-medium" style={{ color: kpi.color }}>{kpi.delta}</p>
          </div>
        ))}
      </div>

      {/* AI Recommendation + Career Progress */}
        ...
        <Card className="relative overflow-hidden">
          ...
          <div className="space-y-3 mb-4">
            ...
            <div className="p-3 rounded-xl border border-accent/20 bg-accent-light/50">
              ...
              <p className="text-sm font-medium text-dark">Завершите <strong>Python ML курс</strong> — самый высокий приоритет для перехода.</p>
            </div>
          </div>
                  ...
                  <button
                    ...
                    className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-medium transition-all border
                      ${draftRole === role
                        ...
                        : 'border-border text-dark hover:border-accent/40 hover:bg-background'}`}
                  >
                    {role}
